...
vector<Cluster> Clustering::calculateClusters(vector<DataPoint> points, LocalSubspaceTable* clusterCandidates, vector<vector<double>> &scoring)
{
	double eps = epsilon;
	int minP = minPoints;
	...
	for (int i = 0; i < clusterCandidates->getTableSize(); i++) 
	{
		...
		score = cv::SilhouetteScore::Overall(data, assignements, metric::EuclideanDistance());
